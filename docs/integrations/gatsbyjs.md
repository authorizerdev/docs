...
Note the `--client-id` value -- you will need it in the SDK configuration below. For more information check [Server Configuration](/core/server-config).
...
> Note: Replace `YOUR_AUTHORIZER_URL` with your authorizer instance URL. Replace `YOUR_CLIENT_ID` with the value of `--client-id` flag used to start the server.
...
## Step 7: Add Authorizer component
...
## Step 8: Add private route layout
