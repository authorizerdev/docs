...
Generates a fresh TOTP secret, QR image, and recovery codes for the caller to enroll as an MFA method — the TOTP twin of [`email_otp_mfa_setup`](#email_otp_mfa_setup) / [`sms_otp_mfa_setup`](#sms_otp_mfa_setup). Unlike those, nothing is sent anywhere: the enrollment payload comes back directly, so the caller scans or enters it, then completes enrollment via [`verify_otp`](#verify_otp) with `is_totp: true`.

Same dual-mode permissions as the other setup mutations: an already-authenticated caller adding a second factor, or a caller in the withheld first-time MFA-offer state identified by the MFA session cookie. Accepts `params` of type `OtpMfaSetupRequest`. Returns `AuthResponse`.
...
Returns the WebAuthn creation options (an opaque JSON string) to pass to the browser's `navigator.credentials.create()`. Takes optional top-level `email` / `phone_number` scalars rather than a `params` object. Returns `WebauthnRegistrationOptionsResponse` (`{ options }`).

### `webauthn_registration_verify`

Submits the credential the browser produced to finish registering a passkey. Accepts `params` of type `WebauthnRegistrationVerifyRequest`. Returns `AuthResponse`: on the MFA-session-cookie path this also completes the gate and issues the withheld access token; on the ordinary authenticated path `access_token` is null.

### `webauthn_login_options`

Returns the WebAuthn request options for `navigator.credentials.get()`. Omit `email` for the usernameless (discoverable credential) ceremony. Returns `WebauthnLoginOptionsResponse` (`{ options }`).

### `webauthn_login_verify`

Submits the signed assertion to complete passkey login. Accepts `params` of type `WebauthnLoginVerifyRequest`. Returns `AuthResponse` with tokens.

### `webauthn_credentials`

Lists the authenticated caller's own registered passkeys. Returns `[WebauthnCredentialInfo!]!`. Requires authentication.
