...
Query to get all the [environment variables](./env).
...
All the [environment variables](./env) values can be obtained using this this.
...
> Note: You will need a SMTP server with an email address and password configured as [authorizer environment](./env) using which system can send emails.
...
Mutation to update [environment variables](./env). It accepts `params` of type `UpdateEnvInput` with keys present in [environment variables](./env)
